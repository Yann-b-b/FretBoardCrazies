...
struct DailyHistoryStoreTests {
    ...
    @Test func firstRecordThenIncrementSameDay() {
        let (store, defaults, suite) = makeStore()
        defer { defaults.removePersistentDomain(forName: suite) }
        store.recordCorrect(now: day1, reactionTime: 2.0, masteredCount: 1)
        #expect(store.todayReps(now: day1) == 1)
        store.recordCorrect(now: day1.addingTimeInterval(60), reactionTime: 4.0, masteredCount: 2)
        #expect(store.todayReps(now: day1) == 2)
    ...
    @Test func newDayStartsFreshRecord() {
        ...
        store.recordCorrect(now: day2, reactionTime: 1.0, masteredCount: 3)
        #expect(store.todayReps(now: day2) == 1)
    ...
    @Test func historySortedAscending() {
        ...
        defer { defaults.removePersistentDomain(forName: suite) }
        let day2 = day1.addingTimeInterval(60 * 60 * 24 + 60)
        store.recordCorrect(now: day2, reactionTime: 1.0, masteredCount: 1)
        store.recordCorrect(now: day1, reactionTime: 1.0, masteredCount: 1)
        let h = store.history()
        #expect(h.count == 2)
