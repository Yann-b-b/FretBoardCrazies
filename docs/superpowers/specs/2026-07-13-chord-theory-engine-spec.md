...
- **OPEN UX DECISION (for the user):** band-7 idiom entries (borrowed-iv/backdoor, turnaround entry) are *generated* and *reachable*, and the idiom **continues** correctly once entered (Fm7→B♭7→I), but from a plain busy tonic they rank below the top-5 (ii–V–I, I→IV/V, secondary dominants, colors all outrank them) — so the engine does not proactively *suggest* the backdoor entry from the tonic. The engine design's tiers imply a "here's what's new" discovery goal. **Decision needed:** should a newly-unlocked niche idiom get a guaranteed discovery slot (e.g. reserve the 5th suggestion for a rule unlocked at the current tier), trading some pure functional-ranking fidelity for teachability? Deferred pending the user's call; current behavior is pure functional ranking (spec-faithful).
